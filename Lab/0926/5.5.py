...
def l1_loss(theta, lambda1):
    ...
    return (
        (theta0 - 2) ** 2
        + (theta1 - 2) ** 2
        + lambda1 * (np.abs(theta0) + np.abs(theta1))
    )
...
ax.clabel(
    contour, inline=True, fontsize=label_fontsize, fmt="%.2f", colors="black"
)  # 更大更清晰
...
ax.clabel(
    contour_J0, inline=True, fontsize=label_fontsize - 2, fmt="%.2f", colors="blue"
)
# L2 正则项等值线（红色虚线圆）
L2_norm = Theta0**2 + Theta1**2
contour_L2 = ax.contour(
    Theta0,
    Theta1,
    L2_norm,
    levels=np.arange(0.5, 3.1, 0.5),  # 更密的正则等值线
    colors="red",
    linestyles="--",
    alpha=0.8,
    linewidths=1.5,
...
ax.clabel(
    contour_L2, inline=True, fontsize=label_fontsize - 2, colors="red", fmt="%.1f"
)
# 最优解
ax.plot(
    theta_l2_0,
    ...
    "r*",
    markersize=14,
    label=rf"$\theta^*_\lambda = ({theta_l2_0:.2f}, {theta_l2_1:.2f})$",
)
ax.plot(2, 2, "k*", markersize=14, label=r"$\theta^* = (2,2)$")
...
contour_L1 = ax.contour(
    Theta0,
    Theta1,
    L1_norm,
    levels=np.arange(0.5, 3.1, 0.5),
    colors="red",
    linestyles="--",
    alpha=0.8,
    linewidths=1.5,
...
ax.clabel(
    contour_L1, inline=True, fontsize=label_fontsize - 2, colors="red", fmt="%.1f"
)
# 最优解
ax.plot(
    theta_l1_0,
    ...
    "r*",
    markersize=14,
    label=rf"$\theta^*_\lambda = ({theta_l1_0:.2f}, {theta_l1_1:.2f})$",
)
ax.plot(2, 2, "k*", markersize=14, label=r"$\theta^* = (2,2)$")
